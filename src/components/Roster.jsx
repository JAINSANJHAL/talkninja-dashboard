...
export default function Roster({ onStudentClick }) {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold" style={{ color: '#1F4D2E' }}>Student Roster</h1>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {stats.map((stat) => (
          <div
            ...
            style={{ backgroundColor: stat.bg, border: `1px solid ${stat.bg}` }}
          >
            <div className="text-3xl mb-3">{stat.emoji}</div>
            <div className="text-3xl font-bold" style={{ color: stat.text }}>{stat.value}</div>
            <div className="text-sm mt-1 font-medium" style={{ color: '#4A4A3F', opacity: 0.75 }}>{stat.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden" style={{ border: '1px solid #E8F5EA' }}>
        <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid #E8F5EA' }}>
          <h2 className="font-semibold" style={{ color: '#1F4D2E' }}>All Students</h2>
          <div className="flex items-center gap-1.5">
            <span className="text-xs mr-2" style={{ color: '#4A4A3F', opacity: 0.5 }}>Sort by:</span>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs" style={{ color: '#4A4A3F', opacity: 0.55 }}>
                          ...
                          {student.streak > 0 && (
                            <span style={{ color: '#8A7200' }}> · 🔥 {student.streak}d</span>
                          )}
                        </p>
                      </div>
                    </div>
                  ...
                  <td className="px-6 py-4">
                    ...
                    >
                      <span
                        ...
                        style={{ backgroundColor: student.active ? '#3D7A4F' : '#B8DBBF' }}
                      />
                      {student.active ? 'Active' : 'Inactive'}
                    </span>
                  </td>

                  <td className="px-6 py-4 text-sm" style={{ color: '#4A4A3F' }}>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-20 h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: '#E8F5EA' }}>
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, (student.sessionsCompleted / 20) * 100)}%`,
                            backgroundColor: '#3D7A4F',
                          }}
                        />
                      </div>
                      <span className="text-sm font-medium" style={{ color: '#1F4D2E' }}>
